...
async def generate_video_prompt(child_input: str, child_profile: dict) -> str:
    ...
    prompt = f"""You are an expert at writing prompts for children's educational 
animated videos. Write vivid, specific, child-friendly prompts.
Respond with ONLY the prompt text, nothing else.

Write a video generation prompt for:
Topic: {child_input}
Child: {name}, age {age}
Style: {style}
...
async def fuzzy_match_video(child_input: str, video_map: dict) -> str:
    ...
    prompt = f"""Match questions to the closest topic. Reply with ONLY the exact topic string, nothing else.

Question: '{child_input}'
Topics: {topics}
Best match:"""
